...
export const register = async (req, res, next) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      // if referredBy set, ensure Referral record exists (idempotent)
      if (createdUser.referredBy) {
        await Referral.findOneAndUpdate(
          { referrerId: createdUser.referredBy, refereeId: createdUser._id },
          {
            $setOnInsert: {
              referrerId: createdUser.referredBy,
              refereeId: createdUser._id,
              level: 1,
              commissionCents: 0,
              paid: false,
              relatedTxId: null,
